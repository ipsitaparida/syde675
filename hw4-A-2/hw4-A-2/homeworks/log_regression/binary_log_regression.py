...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        # temp = self.bias + np.dot(self.weight.T, X.T)
        # mu = 1 / (1 + np.exp(np.multiply (-y, temp.T)))
        mu = 1/(1+np.exp(-y*(self.bias+X.dot(self.weight))))
        return mu
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        e_exp = np.exp(-y*(self.bias+X.dot(self.weight)))
        loss = np.mean(np.log(1+ e_exp)) + self._lambda * np.linalg.norm(self.weight, ord=2)**2
        return loss
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        n = y.size
        mu = self.mu(X, y)
        # J_w = 1/n * np.dot(np.multiply(-y, X).T,1-mu) + 2* self._lambda*self.weight
        return np.mean(-y*(X.T)*(1-mu), axis=1) + 2*self._lambda*self.weight
    ...
    def gradient_J_bias(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        mu = self.mu(X, y)
        # J_b = 1/n * np.dot(-y.T, 1-mu)
        return np.mean(-y*(1-mu))
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        signs = np.sign(X.dot(self.weight)+self.bias)
        predictions = np.where( signs == 0, 1, signs )
        return predictions
    ...
    def misclassification_error(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        error = 1-(np.sum(np.where(predictions!=y, 0, 1))/len(y))
        # error = 1 - np.sum( np.abs(predictions + y) ) / (2*y.size)
        return error
    ...
    def step(self, X: np.ndarray, y: np.ndarray, learning_rate: float = 1e-4):
        ...
        Args:
            ...
            learning_rate (float, optional): Learning rate of SGD/GD algorithm.
                Defaults to 1e-4.
        """
        j_w = self.gradient_J_weight(X, y)
        j_b = self.gradient_J_bias(X, y)
        self.bias -= learning_rate*j_b
        self.weight -= learning_rate*j_w
    ...
    def train(
        ...
        for i in tqdm(range(epochs)):
            for j in range(num_batches):
                ...
                X_t = X_train[list(indices)]
                y_t = y_train[list(indices)]
                # X_t = X_train[[indices]]
                # y_t = y_train[[indices]]
...
if __name__ == "__main__":
    ...
    lr = 0.01
    batch = len(x_train)
    history = model.train(x_train, y_train, x_test, y_test, learning_rate=lr, batch_size=batch, epochs=200)
    plot_obs(history)

    #c
    model = BinaryLogReg(0.1)
    print("For Batch size 1")
    lr = 0.01
    batch = 1
    history = model.train(x_train, y_train, x_test, y_test, learning_rate=lr, batch_size=batch, epochs=200)
    plot_obs(history)
