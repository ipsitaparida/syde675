...
from tag_decorator import ProblemDecorator
...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    B_tp = vanilla_transpose(B)
    A_tp = vanilla_transpose(A)
    comp1 = vanilla_matmul(A, x)
    comp2 = vanilla_matmul(A_tp, x)
    comp3 = vanilla_matmul(B_tp, y)

    soln = [comp1[i] + comp2[i] + comp3[i] for i in range(len(comp1))];
    return soln
...
def numpy_solution(
    ...
    B_tp = np.transpose(B)
    A_tp = np.transpose(A)
    soln = np.matmul(A, x) + np.matmul(A_tp, x) + np.matmul(B_tp, y)
    return soln
